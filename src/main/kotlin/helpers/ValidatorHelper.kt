...
class ValidatorHelper(private val data: Map<String, Any?>) {
    ...
    private fun addError(field: String, error: String) {
    ...
    // Fungsi baru untuk validasi pilihan (LOST/FOUND atau OPEN/RESOLVED)
    fun inList(field: String, list: List<String>, message: String? = null) {
        val value = data[field]
        if (value is String && !list.contains(value)) {
            addError(field, message ?: "$field must be one of: ${list.joinToString(", ")}")
        }
    }
    ...
    fun validate() {
        if (errors.isNotEmpty()) {
            // Menggabungkan error dengan pemisah "|" agar bisa diparsing oleh parseMessageToMap
